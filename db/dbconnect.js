...
const mysql = require('mysql'); //db연결하고 sql쿼리돌려서 원하는 db 빼오기 위해 필요
 //  body-parser -> node 16버전에서는 express로 편입 호출필요없음
const bodyParser = require('body-parser'); //req.body피싱하려고 (풀어헤쳐서보는거)
const router = express.Router();//..기본
const dbconfig = require('./dbconfig'); //db정보 외부로 빼주기 깃허브에 올리지않도록 처리하기 -> gitignore에서 확인

var mybatisMapper = require('mybatis-mapper'); // sql저장해둔  xml 접근 모듈

//web.js를 기준으로 작성하는 것이므로 './mapper/reactSQL.xml' 경로를 ./라고 작성
...
const conn=mysql.createPool(dbconfig)// createPool -> 서버를 동시에 사용할 수 있는 풀장을 만듦(서버접속의 한정을 만듦)
// 서버접속 요청에 일일이 접속하면 자원낭비 / 풀장처럼 담아서 실행
// 1. MySQL 모듈을 불러온다.
// 2. DB와 연결한다
// 3. 쿼리를 실행한다
// 4. 연결을 끊는다. 요걸 매번해야만 한다...
// 그래서! db접속 정보를 createPool로 실행하므로 2가지 옵션 추가
